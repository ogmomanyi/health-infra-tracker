...
(function () {
    ...
    function render() {
        ...
        document.getElementById("procurementTable").innerHTML = `
          <div class="table-wrap"><table><thead><tr><th>Tender</th><th>Buyer</th><th>Country</th><th>Product</th><th>Closing</th><th>IATI</th><th>Confidence</th><th>Status</th></tr></thead>
          <tbody>${rows.slice(0, 100).map(e => `<tr><td>${esc(e.title)}</td><td>${esc(e.buyer)}</td><td>${esc(e.country)}</td><td>${esc(e.product_family || e.equipment_category)}</td><td>${esc(e.closing_date)}</td><td>${esc(e.matched_iati_identifier || "—")}</td><td>${esc(e.match_confidence)}%</td><td>${esc(e.match_status)}</td></tr>`).join("")}</tbody></table></div>`;
    }
