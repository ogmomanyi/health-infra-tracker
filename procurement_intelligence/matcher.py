...
def match_event(event, projects, threshold=65.0):
    """Match an external procurement event to IATI using evidence only."""
    ...
    for project in projects:
        ...
        score = round(title_score * 0.65 + buyer_score * 0.20 + (10.0 if country_match else 0.0) + (5.0 if equipment_match else 0.0), 1)
        if not (title_score >= 55.0 or buyer_score >= 70.0):
            continue
