...
def _catalogue_manufacturers(path: Path) -> dict[str, set[str]]:
    ...
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        for row in rows:
            ...
            if not manufacturer:
                continue
            result.setdefault(_norm(manufacturer), set()).add(
                " ".join((row.get("product_name") or "").split())
            )
        return result


def build_manufacturer_history(
    ...
    for group in groups.values():
        ...
        rows.append({
            ...
            "suppliers": _join(group["suppliers"]),
            "buyers": _join(group["buyers"]),
            "countries": _join(group["countries"]),
            "categories": _join(group["categories"]),
            ...
            "faram_catalogue_status": catalogue_status,
            "faram_products": "; ".join(products),
            "competitive_gap": gap,
