    ...
    def do_GET(self):
        try:
            ...
            if path in {"/accounts", "/accounts.html"}:
                return self._serve(ACCOUNT_HTML)
            if path in {"/management", "/management.html"}:
            ...
            if len(parts) == 4 and parts[:2] == ["api", "opportunities"]:
                oid = parts[2]
                if commercial_crm.get_opportunity(oid, db_path=self.db_path) is None:
                    return self._json(404, {"error": "opportunity not found"})
                if parts[3] == "activities":
                    return self._json(200, commercial_crm.list_activities(oid, db_path=self.db_path))
                if parts[3] == "audit":
