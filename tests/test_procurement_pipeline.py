...
class ProcurementPipelineTests(unittest.TestCase):
    def _event(self, event_id, title):
        return ProcurementEvent(
            event_id,
        )

    def test_persist_events_creates_and_replaces_snapshot(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                title = conn.execute("select title from procurement_intelligence").fetchone()[0]
            finally:
                conn.close()
        self.assertEqual(count, 1)
        self.assertEqual(title, "Lab equipment")

    def test_deduplicate_events_by_stable_id(self):
        event = ProcurementEvent(
            "proc_1", "UNGM", "", "A-1", "Lab equipment", "WHO", "Kenya", "", "",
            "Laboratory Equipment", "Analyzers",
        )
        duplicate = ProcurementEvent(
            "proc_1", "UNGM", "", "A-1", "Updated title", "WHO", "Kenya", "", "",
            "Laboratory Equipment", "Analyzers",
        )
...
,UNGM,https://example.test,A-1,Kenya lab analyzer procurement,WHO,Kenya,2026-09-01,2026-10-01,Laboratory Equipment,Analyzers,100000,USD,,0,UNMATCHED
"""
        with tempfile.TemporaryDirectory() as tmp:
            data_dir = Path(tmp)
        ...
        self.assertEqual(count, 1)
        self.assertIn("procurement_event_id", output.splitlines()[0])
